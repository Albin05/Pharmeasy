import { Link } from "react-router-dom";
import CenterMode from "./Carousel";

export const Productsmain = () => {
  return (
    <div>
      <div className="main-div">
        <div className="location">Home {`>`} Products</div>
        <CenterMode />
        <div className="product-categories">
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/1279753fc985369b8a8a5069f7fbe209.jpg"
                alt=""
              />
              <h4>Personal Care</h4>
              <div>
                <p>Upto</p>
                <p>80% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/b8832bde65de3cafbd8a84861b9b9d1d.jpg"
                alt=""
              />
              <h4>Health Food and Drinks</h4>
              <div>
                <p>Upto</p>
                <p>57% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/8df738c0349d304480de421fd6c6c24f.jpg"
                alt=""
              />
              <h4>Skin Care</h4>
              <div>
                <p>Upto</p>
                <p>50% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/47031d01e7ea3e1ead70342da6ace7ad.jpg"
                alt=""
              />
              <h4>Home Care</h4>
              <div>
                <p>Upto</p>
                <p>35% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/25499457d849302885780222cd52b237.png"
                alt=""
              />
              <h4>Ayurvedic Care</h4>
              <div>
                <p>Upto</p>
                <p>70% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/7be7b3ff31bf3d1bb5d374d984ddfc8b.jpg"
                alt=""
              />
              <h4>Sexual Wellness</h4>
              <div>
                <p>Upto</p>
                <p>53% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/321bf75f9b693920b1a5a280b8db85e1.jpg"
                alt=""
              />
              <h4>Fitness & Supplements</h4>
              <div>
                <p>Upto</p>
                <p>80% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/b334c54f69763e129b4cc265de5d74a3.jpg"
                alt=""
              />
              <h4>Mother and Baby Care</h4>
              <div>
                <p>Upto</p>
                <p>50% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/b75b42b41fb83d5f926b42ea77cb8035.jpg"
                alt=""
              />
              <h4>Healthcare Devices</h4>
              <div>
                <p>Upto</p>
                <p>65% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/e63d776d17433b73ab220cb1d6e8ae19.jpg"
                alt=""
              />
              <h4>Surgicals and Dressings</h4>
              <div>
                <p>Upto</p>
                <p>55% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/0193fd80acd733bca44ea1fc3bfd74b3.jpg"
                alt=""
              />
              <h4>Covid Essentials</h4>
              <div>
                <p>Upto</p>
                <p>77% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/6e38477688f1318a894ac3578ec544f9.jpg"
                alt=""
              />
              <h4>Health Condition</h4>
              <div>
                <p>Upto</p>
                <p>65% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/93445039fa903392a6b6e3cc1cf58b81.jpg"
                alt=""
              />
              <h4>Diabetic Care</h4>
              <div>
                <p>Upto</p>
                <p>65% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/249fbf870f1b3c3a9fc7883f3828b137.jpg"
                alt=""
              />
              <h4>Elderly Care</h4>
              <div>
                <p>Upto</p>
                <p>45% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/00eeede7f4a735259796382c8a19723e.png"
                alt=""
              />
              <h4>Accessories And Wearables</h4>
              <div>
                <p>Upto</p>
                <p>35% off</p>
              </div>
            </div>
          </a>
          <a href="">
            <div className="product-single">
              <img
                src="https://cdn01.pharmeasy.in/dam/discovery/categoryImages/9bc9c511ab293f1686007cf670ce6082.png"
                alt=""
              />
              <h4>Pet Care</h4>
              <div>
                <p>Upto</p>
                <p>30% off</p>
              </div>
            </div>
          </a>
        </div>
        <div className="text-content">
          <h4>Shop Safely And Worry-Free On PharmEasy</h4>
          <p>
            PharmEasy is a renowned name in the online shopping of Over The
            Counter or healthcare products and other daily essentials. Ordering
            online on PharmEasy is convenient, fast and hassle-free; you can
            avoid the annoyance of queuing up at your local pharmacy and also
            avail of huge discounts of up to 80% OFF. Purchasing the healthcare
            requirements such as medical devices and nutritional supplements
            that you need will no longer burn a hole in your pocket. At
            PharmEasy, you are sure to find everything that you need because we
            have products across all healthcare categories.
          </p>

          <h4>Why choose PharmEasy?</h4>
          <ul>
            <li>8400+ Brands</li>
            <li>35K products in our stock</li>
            <li>Delivery to 22000+ pin codes</li>
            <li>Delivered to 50 lakh+ families</li>
            <li>Over 2M customers served</li>
          </ul>

          <h4>Our most popular categories:</h4>
          <p>We have a range of categories and products, some which are:</p>

          <h4>Devices</h4>
          <p>
            With the pandemic upending our lives, we have all realized that
            there are some medical devices that we need to keep in our homes
            such as Oximeter, Thermometer, Inhaler, Blood pressure monitor,
            Glucometer.
          </p>
          <h4>Personal care</h4>
          <p>
            Personal care products can include a wide range of products such as
            Bath products, , Skincare essentials, Razor blades, Toothpaste and
            toothbrush, Wet wipes, Lip balm, Face masks, Hand sanitizer, etc.
          </p>
          <h4>Nutrition and fitness supplements</h4>
          <p>
            To give your health a helping hand, you can order a wide variety of
            health supplements including Health drinks, Nutritional powder,
            Nutrition tablets/capsules.
          </p>
          <h4>Ayurvedic care</h4>
          <p>
            Place your trust in the goodness of natural and organic ayurvedic
            products for an all-around healthy mind and body. There’s plenty to
            choose from such as Aloe vera juice, Moringa capsules, Garlic
            capsules.
          </p>
          <h4>Home care</h4>
          <p>
            Home care products are things you need every day such as
            disinfectant spray, phenyl, dishwashing liquid, etc. Get them all at
            spectacular discounts on PharmEasy.
          </p>
          <h4>Baby and mom care</h4>
          <p>
            PharmEasy also has an impressive range of baby and mom care products
            including Diapers, Baby bath products, Infant formula food, Mother’s
            health drinks, Diaper rash creams, Baby wipes.
          </p>
          <h4>Our top brands:</h4>
          <p>
            Get products from the most famous and trusted brands including
            Accu-chek, Dettol, Horlicks, Baidyanath, Sebamed.
          </p>
          <h4>You are safe with PharmEasy:</h4>
          <p>
            Given the times, we are taking all precautions to ensure that we
            keep everyone safe - our customers and the people we work with.
          </p>
          <h4>We ensure</h4>
          <ul>
            <li>Regular sanitization of our premises and warehouses</li>
            <li>Temperature checks of our employees and delivery partners</li>
            <li>No-contact door-step delivery</li>
            <li>Sanitized packaging</li>
          </ul>
          <h4>Youe safety is our priority</h4>
          <p>
            Stay safe at home and shop at your convenience from PharmEasy. All
            you have to do is place your order and we will try to deliver it at
            the earliest.
          </p>
        </div>
      </div>
    </div>
  );
};
